...
extension DirectoryWatcher {
  private func directoryMetadata(_ url: URL) -> [String]? {
    ...
    for filename in contents {
      ...
      guard
        ...
        let fileSize = fileAttributes[.size] as? Double
      else {
        continue
      }
  ...
  private func pollDirectoryForChangesWith(_ oldMetadata: [String]) {
    guard let newDirectoryMetadata = self.directoryMetadata(self.watchedUrl)
    else {
      return
    }
      ...
      DispatchQueue.main.async {
        ...
        if !deletedElements.isEmpty {
          let elements = deletedElements.compactMap({ (element) -> URL? in
            let isDirectory =
              (try? element.resourceValues(forKeys: [.isDirectoryKey]))?
              .isDirectory ?? false
